...
function (data.path, use.names = FALSE, assay = NULL) 
{
    data <- if (dir.exists(data.path)) {
        Read10X(data.path, gene.column = ifelse(use.names, 2, 
            1))
    }
    else {
        Read10X_h5(data.path, use.names = use.names)
    }
    if (class(data) == "list") {
...
function (font_use = "Arial") 
{
    ...
    mytheme <- theme_bw() + theme(panel.grid = element_blank(), 
        ...
        axis.text.y = element_text(hjust = 1, vjust = 0.5), axis.title = element_text(color = "#000000", 
            ...
            size = 0.5), axis.ticks.length = unit(0.1, "cm"), 
...
function (font_use = "Arial") 
{
    ...
    mytheme <- theme_bw() + theme(panel.grid.major = element_blank(), 
        panel.grid.minor = element_blank(), panel.border = element_rect(color = "#000000", 
            size = 0.8), axis.text = element_text(color = "#000000", 
    ...
    maxColorValue = 255) 
{
    ...
    Y <- switch(algorithms, luminance = 0.299 * red + 0.587 * 
        green + 0.114 * blue, luma = 0.2126 * red + 0.7152 * 
        green + 0.0722 * blue, average = mean(c(red, green, blue)), 
        desaturation = (max(red, green, blue) + min(red, green, 
            blue))/2, max = max(red, green, blue), min = min(red, 
            green, blue), red = red, green = green, blue = blue)
...
function (object, parameter = list(), group.by = "seurat_clusters", 
    min.pct = IfNull(parameter$FindMarkers$min_pct, 0.25), logfc.threshold = IfNull(parameter$FindMarkers$logfc, 
        ...
        0.01), pseudocount.use = IfNull(parameter$FindMarkers$pseudocount.use, 
        0), only.pos = TRUE, ...) 
{
    Idents(object) <- group.by
...
{
    message("-->Run tSNE<--")
    perplexity <- min(30, floor(length(dims) - 1)/3)
...
FindMarkers.default <-
function (object, slot = "data", counts = numeric(), cells.1 = NULL, 
    ...
    is.use.name = TRUE, ...) 
{
    ...
    if (plot.feature) {
        PlotFeaturePlot(object, features = features, outfile = paste0(outpref, 
            ".Distribution.pdf"), reduction = "tsne", is.use.name = is.use.name)
    }
}
PlotBasicStat <-
function (object, outpref = NULL, color = NULL, assay = NULL, 
    group.by = "orig.ident", span = NULL, stat_mito = TRUE, standard = NULL, 
    ...) 
{
    if (is.null(color)) {
    ...
    get_y_name <- function(feature) sapply(features, switch, 
        percent.mito = "Percentage(%)", "Number")
    ...
    cols.use = NULL, is.use.name = TRUE, ...) 
{
    ...
    VlnplotShell(object, features, split.save.pref = outpref, 
...
function (object, features = NULL, group.by = "orig.ident", cols.use = NULL, 
    ...
    split.save.pref = NULL, pt.size = 0.1, alpha = 0.5, is.cline.x.text = TRUE, 
    standard = NULL, ...) 
{
    if (is.null(features)) 
        return(1)
